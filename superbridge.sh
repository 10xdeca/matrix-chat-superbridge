...
VPS_HOST="${VPS_HOST:-nick@35.201.14.61}"
VPS_DIR="${VPS_DIR:-~/matrix-chat-superbridge}"
SERVER_NAME="${SERVER_NAME:-35-201-14-61.sslip.io}"
ADMIN_USER="${ADMIN_USER:-nick}"
ROOM_NAME="${ROOM_NAME:-xdeca Superbridge}"
...
# State file on the VPS to track superbridge room ID
STATE_FILE="$VPS_DIR/.superbridge-state"
...
get_access_token() {
  ...
  # Check if token is saved on VPS
  local saved_token
  saved_token=$(ssh_vps "grep ACCESS_TOKEN $STATE_FILE 2>/dev/null | cut -d= -f2" 2>/dev/null || echo "")
  ...
  echo "    curl -s -X POST 'http://localhost:6167/_matrix/client/v3/login' \\" >&2
  ...
  ssh_vps "touch $STATE_FILE && chmod 600 $STATE_FILE && echo 'ACCESS_TOKEN=$token' >> $STATE_FILE" 2>/dev/null || true
}

# Make an authenticated Matrix API call on the VPS.
...
matrix_api() {
  ...
  if [[ -n "$data" ]]; then
    echo "$data" | ssh_vps "curl -sf -X '$method' 'http://localhost:6167${endpoint}' \
      -H 'Authorization: Bearer $token' \
      -H 'Content-Type: application/json' \
      -d @-"
  else
    ssh_vps "curl -sf -X '$method' 'http://localhost:6167${endpoint}' \
      -H 'Authorization: Bearer $token' \
      -H 'Content-Type: application/json'"
  fi
}
...
# Get saved room ID
get_room_id() {
  ssh_vps "grep ROOM_ID $STATE_FILE 2>/dev/null | cut -d= -f2" 2>/dev/null || echo ""
}
...
create_room() {
  ...
  if [[ -n "$existing_room" ]]; then
    ...
    warn "Delete $STATE_FILE on VPS to start over, or use 'invite-bots' to continue."
    return
  fi
...
EOF
  ...
  ssh_vps "mkdir -p \$(dirname $STATE_FILE) && install -m 600 /dev/null $STATE_FILE && echo 'ROOM_ID=$room_id' > $STATE_FILE"
...
plumb_telegram() {
  ...
  echo "    Find your target group in the list."
...
plumb_whatsapp() {
  ...
  echo "    Find your target group and note the JID"
...
signal_note() {
  ...
  echo "    2. Use the relay-bot appservice to bridge the Signal portal room into the hub"
  ...
  info "WhatsApp is experimental. Signal stays separate (or use the relay bot)."
}
