...
class RelayHandler:
    ...
    async def _resolve_double_puppet(
        ...
        For a user like ``@nick:domain`` in a Discord portal room, finds the
        matching Discord puppet (e.g. ``@discord_123:domain``) and returns
        its display name and avatar.

        The correct puppet is identified by matching the platform prefix
        of the portal room (e.g. ``discord_``, ``telegram_``) against the
        configured puppet MXIDs.
        """
        portal_label = self._portal_rooms[room_id]  # e.g. "Discord", "Telegram"
        prefix = portal_label.lower() + "_"  # e.g. "discord_", "telegram_"
